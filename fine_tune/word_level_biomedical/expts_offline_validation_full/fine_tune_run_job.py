...
def main(model_name_or_path, train_file_path, eval_file_path, debug):
    ...
    for comb in combinations:
        ...
        error_file = os.path.join(file_dir, SCRIPT_FILE, f'{output_dir}.error')
        with open( bash_file, 'w' ) as OUT:
            ...
            OUT.write(command)
        print(command)
        qsub_command = f'qsub -P medifor -q all.q -j y -o {dest_file} -e {error_file} -l h_rt=24:00:00,h_vmem=20g,gpu=1 {bash_file}'
        print(qsub_command)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_file_path', default="../bio_medical_data_demo/train_COVID.txt")
    parser.add_argument('--eval_file_path', default="../bio_medical_data_demo/val_COVID.txt")
