...
class TextDataset(Dataset):
    ...
    # in this way, how to set up the shuffle=None here?
    # how to select this index here?
    # index ==> slice_num + sample number
...
def _sorted_checkpoints(args, checkpoint_prefix="checkpoint-current", use_mtime=False) -> List[str]:
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    cur_gas_loss = (tr_loss-logging_loss) / args.logging_steps   # the loss during the last gradient_accumulation_step
                    cur_gas_perplexit = np.exp(cur_gas_loss)
                    tb_writer.add_scalar("train/iter_loss", cur_gas_loss, global_step)
                    tb_writer.add_scalar("train/iter_preplexity", cur_gas_perplexit, global_step)
                    tb_writer.add_scalar("progress/lr", scheduler.get_lr()[0], global_step)
                    logging_loss = tr_loss

                    ## save and evaluation.
                    args.global_step = global_step
                    current_prefix = "checkpoint-current-" + str(global_step)
                    ...
                    # offline does not need to rm. the previous checkpoint.  
            ...
            if args.debug and step == 20:
                print(f"global_step is {global_step}, step is {step}.", flush=True)
                break
        ...
        tb_writer.add_scalar("train/epoch_preplexity", cur_epoch_prelexity, global_step)
...
def main():
    ...
    parser.add_argument("--logging_steps", type=int, default=15000, help="Log every X updates steps.")
    parser.add_argument(
    ...
    if args.should_continue:
        ...
        if len(sorted_checkpoints) == 0:
            ...
            # args.model_name_or_path = sorted_checkpoints[-1]  the last one may be problematic here.
            args.model_name_or_path = sorted_checkpoints[-2]
    ...
    ## step 5: train and save the model after the training
    # Training
    if args.do_train:
        if args.local_rank not in [-1, 0]:
            torch.distributed.barrier()  # Barrier to make sure only the first process in distributed training process the dataset, and the others will use the cache

        # print(f"staring loading the dataset.")
        train_dataset = load_and_cache_examples(args, tokenizer, evaluate=False)
        # print(f"the dataset length is: {len(train_dataset)}.")
        # import sys;sys.exit(0)
        # for idx, batch_cur in enumerate(train_dataset):
        #     print(batch_cur.size())
        # print(f"finsihing loading the dataset.")
        # import sys;sys.exit(0)
